    ...
    <nav className="bg-white h-[88px] fixed top-0 w-full z-50 shadow-md flex items-center">
      <div className="max-w-[1280px] mx-auto flex justify-between items-center px-6 w-full">
        ...
        <button className="md:hidden" onClick={() => setIsOpen(!isOpen)}>
          {isOpen ? <FaTimes className="text-2xl text-[#6C5332]" /> : <FaBars className="text-2xl text-[#6C5332]" />}
        </button>
      </div>
...
const NavItem = ({ link, isActive }) => (
  <Link
    ...
    className={`text-lg font-medium transition ${
      isActive ? "text-[#6C5332] font-bold border-b-2 border-[#D4AF37]" : "text-[#6C5332] hover:text-[#D4AF37]"
    }`}
  >
    {link.name}
  </Link>
